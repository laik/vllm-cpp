"""Test full forward pass with correct RoPE layout [B, heads, seq, dim]."""
...
# Tokenizer (simplified - just use token IDs directly)
...
# ============================================================
# Forward with FIXED RoPE layout
# ============================================================
def forward_fixed(input_ids):
    """RoPE in [B, heads, seq, dim], SDPA in [B, seq, heads, dim]."""
    ...
    for l in range(num_layers):
        ...
        # RoPE requires [B, heads, seq, dim] — axis -2 must be seq_len
        q = mx.transpose(q, (0, 2, 1, 3))  # [B, seq, heads, dim] → [B, heads, seq, dim]
        ...
        q = mx.transpose(q, (0, 2, 1, 3))  # [B, heads, seq, dim] → [B, seq, heads, dim]
        k = mx.transpose(k, (0, 2, 1, 3))

        # SDPA with per-group GQA
...
# Test
print("========== Fixed RoPE Layout ==========")
test_tokens = list(tokens)

logits = forward_fixed(test_tokens)
n0 = show_top5(logits, f"Step 0 (seq={len(test_tokens)})")
print(f"  Greedy: {n0} ({inv_vocab.get(n0, '?')!r})")

test_tokens.append(n0)
logits = forward_fixed(test_tokens)
n1 = show_top5(logits, f"Step 1 (seq={len(test_tokens)})")
print(f"  Greedy: {n1} ({inv_vocab.get(n1, '?')!r})")

test_tokens.append(n1)
logits = forward_fixed(test_tokens)
n2 = show_top5(logits, f"Step 2 (seq={len(test_tokens)})")
print(f"  Greedy: {n2} ({inv_vocab.get(n2, '?')!r})")

test_tokens.append(n2)
logits = forward_fixed(test_tokens)
n3 = show_top5(logits, f"Step 3 (seq={len(test_tokens)})")
print(f"  Greedy: {n3} ({inv_vocab.get(n3, '?')!r})")

test_tokens.append(n3)
logits = forward_fixed(test_tokens)
n4 = show_top5(logits, f"Step 4 (seq={len(test_tokens)})")
print(f"  Greedy: {n4} ({inv_vocab.get(n4, '?')!r})")

print("\nDecoded text so far:", "".join(inv_vocab.get(t, f"<{t}>") for t in test_tokens[len(tokens):]))
